...
public:
    ...
    explicit Regex(Regex* ptr)
            : ptr_(ptr) { }

    Regex(Regex&& regex) noexcept
            : ptr_(std::move(regex.ptr_)) { }

    explicit Regex(std::unique_ptr<Regex> ptr)
            : ptr_(std::move(ptr)) { }
    ...
    Regex* get() noexcept { return ptr_.get(); }

    const Regex* get() const noexcept { return ptr_.get(); }

private:
    std::unique_ptr<Regex> ptr_;
};
